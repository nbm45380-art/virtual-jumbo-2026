(()=>{
  ...
  const bases=[
    ...
    `https://raw.githubusercontent.com/nbm45380-art/virtual-jumbo-2026/loto-public-apps-v3/${path}`
  ];
  ...
  async function loadScript(name,n,total){
    let last;
    for(let b=0;b<bases.length;b++){
      try{
        setText('freshMeta',`完全版データ ${n}/${total} を読み込み中… 接続先${b+1}`);
        await tryScript(bases[b]+name,n,total);
        return;
      }catch(e){last=e;}
    }
    throw new Error(`完全版データ ${n}/${total} を取得できませんでした`);
  }
  async function boot(){
    ...
    try{
      ...
      for(let i=0;i<files.length;i++) await loadScript(files[i],i+1,files.length);
